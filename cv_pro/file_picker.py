...
class FilePicker:
    ...
    def __init__(self, root):
        ...
        ----------
        root : string
            A file path to a directory containing .bin files.
        ...
        # Get names of subfolders containing .bin files
        ...
        for file in os.listdir(self.root):
            if file.endswith('.bin'):
                self.file_list[-1][1].append(file)
        # Pop if no .bin files direcly in the root directory
        if self.file_list[-1][1] == []:
            self.file_list.pop(-1)
